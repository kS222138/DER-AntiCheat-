...
var _log_encryptor
var _preset_loader = null

const PLUGIN_VERSION = "2.3.0"
const PRESET_DIR = "res://addons/DER AntiCheat /config/presets/"
...
func _enter_tree():
	...
	print("\n🛡️ DER AntiCheat v%s Enabled | Performance Optimized | Mobile Compatible\n" % PLUGIN_VERSION)
...
func _load_preset_system():
	var preset_script = load(PRESET_DIR + "protection_preset.gd")
	if preset_script:
		_preset_loader = preset_script.new()
		logger.info("plugin", "Preset system loaded")
	else:
		push_warning("DER AntiCheat: Preset system not found at %s" % PRESET_DIR)

func load_preset(preset_name: String) -> bool:
	if not _preset_loader:
		return false
	
	var preset_path = PRESET_DIR + preset_name.to_lower() + "_protection.tres"
	if not FileAccess.file_exists(preset_path):
		push_error("DER AntiCheat: Preset not found: %s" % preset_path)
		return false
	...
	logger.info("plugin", "Preset loaded: %s" % preset_name)
	return true

func _apply_preset(preset) -> void:
	if not preset:
		return
	
	for prop in preset.get_property_list():
		var prop_name = prop.name
		if prop_name.begins_with("_") or prop_name in ["script", "preset_name", "preset_description", "preset_icon"]:
			continue
		
		if prop_name.ends_with("_enabled") and prop.type == TYPE_BOOL:
			var module_name = prop_name.replace("_enabled", "")
			var value = preset.get(prop_name)
			_set_module_enabled(module_name, value)

func _set_module_enabled(module_name: String, enabled: bool) -> void:
	var module_var = "_" + module_name
	if module_name in self:
		var module = self.get(module_name)
		if module and module.has_method("set_enabled"):
			module.set_enabled(enabled)

func quick_setup(game_type: String = "singleplayer") -> bool:
	match game_type.to_lower():
		"singleplayer", "单机":
			return load_preset("light")
		"multiplayer", "联机":
			return load_preset("standard")
		"competitive", "竞技":
			return load_preset("competitive")
	return false
